...
def generate_report(results, user_need, llm_result):
    ...
    try:
        # 确保所有输出使用UTF-8编码
        print("\n=== 开源项目智能选型报告 ===".encode('utf-8', errors='ignore').decode('utf-8'))
        print(f"用户需求: {user_need}".encode('utf-8', errors='ignore').decode('utf-8'))
        
        # 显示系统理解的用户需求
        print("\n系统理解的需求:".encode('utf-8', errors='ignore').decode('utf-8'))
        print(f"- 需求定位: {llm_result.get('task_type', '未知')}".encode('utf-8', errors='ignore').decode('utf-8'))
        print("- 关键需求点:".encode('utf-8', errors='ignore').decode('utf-8'))
        for req in llm_result.get('key_requirements', []):
            if req:
                # 处理编码问题，确保中文显示正常
                if isinstance(req, str):
                    req = req.encode('utf-8', errors='ignore').decode('utf-8')
                print(f"  * {req}".encode('utf-8', errors='ignore').decode('utf-8'))
        
        # 显示评估维度与权重
        print("\n评估维度与权重:".encode('utf-8', errors='ignore').decode('utf-8'))
        weights = llm_result.get('weights', {})
        for dimension, weight in weights.items():
            # 处理编码问题，确保中文显示正常
            if isinstance(dimension, str):
                dimension = dimension.encode('utf-8', errors='ignore').decode('utf-8')
            print(f"- {dimension}: {weight:.2f}".encode('utf-8', errors='ignore').decode('utf-8'))
        
        print("\n适合的项目排名:".encode('utf-8', errors='ignore').decode('utf-8'))
        ...
        for i, result in enumerate(results, 1):
            ...
            try:
                ...
                print(f"\n{i}. {name}".encode('utf-8', errors='ignore').decode('utf-8'))
                print(f"   总分: {total_score:.3f}".encode('utf-8', errors='ignore').decode('utf-8'))
                # 处理编码问题，移除无法编码的字符
                description = project.get('description', '无')
                if isinstance(description, str):
                    description = description.encode('utf-8', errors='ignore').decode('utf-8')
                print(f"   描述: {description}".encode('utf-8', errors='ignore').decode('utf-8'))
                if 'html_url' in project:
                    print(f"   链接: {project['html_url']}".encode('utf-8', errors='ignore').decode('utf-8'))
                
                # 显示各维度得分
                print("   各维度得分:".encode('utf-8', errors='ignore').decode('utf-8'))
                for dimension, score in dimension_scores.items():
                    print(f"   - {dimension}: {score:.3f}".encode('utf-8', errors='ignore').decode('utf-8'))
            except Exception as e:
                print(f"\n{i}. 项目信息处理失败: {e}".encode('utf-8', errors='ignore').decode('utf-8'))
    except Exception as e:
        print(f"生成报告失败: {e}".encode('utf-8', errors='ignore').decode('utf-8'))
        print("   迁移成本评估: 低".encode('utf-8', errors='ignore').decode('utf-8'))
        
        # 风险提示
        print("   风险提示:".encode('utf-8', errors='ignore').decode('utf-8'))
        print("   - 依赖管理".encode('utf-8', errors='ignore').decode('utf-8'))
        print("   - 版本兼容性".encode('utf-8', errors='ignore').decode('utf-8'))
        
        # 趋势预测
        print("   趋势预测: 稳定".encode('utf-8', errors='ignore').decode('utf-8'))
    
    # 最终推荐结论
    print("\n最终推荐结论:".encode('utf-8', errors='ignore').decode('utf-8'))
    if results:
        top_project = results[0]['project']
        print(f"推荐使用: {top_project['name']}".encode('utf-8', errors='ignore').decode('utf-8'))
        print("推荐理由:".encode('utf-8', errors='ignore').decode('utf-8'))
        print(f"- 综合评分最高: {results[0]['total_score']:.3f}".encode('utf-8', errors='ignore').decode('utf-8'))
        print("- 最符合您的需求:".encode('utf-8', errors='ignore').decode('utf-8'))
        for req in llm_result.get('key_requirements', []):
            if req:
                print(f"  * {req}".encode('utf-8', errors='ignore').decode('utf-8'))
